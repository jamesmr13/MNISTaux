function mnist_2categories_quadratic_NLLS_SGD()
close all
fsz = 20;
...
batch_size=256;
%
[w,f,gnorm] = SGD(Xtrain,label,w,batch_size,no_epochs,tol);
figure;
plot(f,'Linewidth',2);
...
function [w,fvals,gnorm] = SGD(X,y,w,batch_size,no_epochs,tol)
    ...
    for k=1:no_epochs
        ...
        while b < N
            ...
            g = qlossgrad(curr_ind,X,y,w,lam);
            w = w - alpha*g;
            b = b + batch_size;
        end
        curr_loss = qloss(1:N,X,y,w,lam);
        % only update w if loss improves
